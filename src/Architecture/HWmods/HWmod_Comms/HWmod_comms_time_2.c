...
unsigned int flag_receive_frame;         /*Flag to comunicate the HWmod that the frame has arrived or not. arrive=1, not arrive=0;*/
...
int main()
{
    ...
    noerror = sdb_register_callback(COMMAND_FLAG_RECEIVE, flag_receive);
    ...
    while(run_in_out == 1) {
        ...
        while(state == STATE_RUN_CONNECT) {          
            ...
            if(type_of_frame == I_HAND) {
                ...
                }else { res_send = send_frame(RR, NULL, 0); }
...
If the send frame is correct return noerror, if not, error*/
    ...
    mcs_data_size = info_size + 17;    
    pkt_in = mcs_create_packet_with_dest(COMMS, &dest, 1, &command, mcs_data_size, data);
...
Receive one frame. Return type of frame or error*/
    ...
    flag_receive_frame = 0; /*To reestart the flag. This is if the 
    
 
    pkt_in = mcs_create_packet_with_dest(COMMS, &dest, 1, &command, 0, NULL);/*Say to ARDUINO receive */

    /*CONTROL OF THE TIME*/
    noerror = sdb_send_async_and_free(pkt_in, &pkt_out_async);
    ...
    if(noerror != NOERROR_INT) {
        ...
        while(time_def < timeout.t_frame && flag_receive_frame != 1) {
            res_time= gettimeofday(&time, NULL);
        ...
        if(flag_receive_frame == 1) {
            flag_receive_frame = 0;/*Put the flag un 0*/
            /*Say to Arduino send the frame to the HWmod*/
            command = GIVE_ME_THE_PACKET;
            pkt_in = mcs_create_packet_with_dest(COMMS, &dest, 1, &command, 0, NULL);
            noerror = sdb_send_sync(pkt_in, &pkt_out); /*Study what is happened if there are ERROR*/
            ...
            if(res != NO_DIRECT && type_of_frame == UNKNOWN && res != FCS_ERROR) {
                ...
                if(type_of_frame == I_HAND) {
                    frames.information[0] = pkt_out->data[0];
                    frames.information[1] = pkt_out->data[1];
                    frames.information[2] = pkt_out->data[2];
				/*If the type of frame is I, put the information in the struct of frame*/
                }else if(type_of_frame == I) {
                    for(i = 0; i < size_data_info; i++) {
                        frames.information[i] = pkt_out->data[i+17];
                    }
                }
...
void flag_receive(MCSPacket *pkt_in, MCSPacket **pkt_out)
{
/*This function is called by ARDUINO to notify the system that has arrived one frame*/
